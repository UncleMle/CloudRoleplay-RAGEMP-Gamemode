...
export default class VehicleEnterSystem {
    ...
    constructor() {
        mp.events.add("render", VehicleEnterSystem.handleRender);
    }
    ...
    private static handleRender() {
        ...
        let veh: VehicleMp = closestVeh[0];

        VehicleEnterSystem.bones.forEach(bone => {
            let bonePos: Vector3 = veh.getWorldPositionOfBone(veh.getBoneIndexByName(bone));

            if(!bonePos) return;

            mp.game.graphics.drawText(
                bone,
                [bonePos.x, bonePos.y, bonePos.z + 0.09],
                {
                    font: 4,
